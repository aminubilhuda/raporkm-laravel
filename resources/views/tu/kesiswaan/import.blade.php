...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
        <h1 class="text-2xl md:text-3xl font-extrabold text-teal-primary-dark flex items-center gap-2"><x-heroicon-o-arrow-up-tray class="w-7 h-7" /> Import Siswa</h1>
        <p class="mt-1 text-sm text-gray-500">Import data siswa dari file CSV atau XLSX.</p>
    </div>
    <a href="{{ route('tu.kesiswaan.index') }}" class="btn-primary inline-flex items-center gap-2 whitespace-nowrap"><x-heroicon-o-arrow-left class="w-5 h-5" /> Kembali</a>
</div>
...
<div class="bg-white rounded-card shadow-card p-5 md:p-6 border-l-[6px] border-l-teal-primary">
    ...
    <form method="POST" action="{{ route('tu.kesiswaan.do-import') }}" enctype="multipart/form-data" class="space-y-4">
        @csrf
        <div>
        ...
        <div>
            ...
            <select name="kelas_id" class="block w-full border-teal-primary/20 rounded-card" required>
                ...
                @foreach($kelass as $k)<option value="{{ $k->id }}">{{ $k->nama_kelas }}</option>@endforeach
            </select>
            @error('kelas_id')<p class="text-coral text-sm mt-1">{{ $message }}</p>@enderror
        </div>
