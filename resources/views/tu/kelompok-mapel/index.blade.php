...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-card shadow-card p-5 md:p-6 border-l-[6px] border-l-teal-primary">
        ...
        <form method="POST" action="{{ route('tu.kelompok-mapel.store') }}" class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            @csrf
            <div>
                <x-input-label for="nama" value="Nama Kelompok" />
                <x-text-input id="nama" name="nama" :value="old('nama')" placeholder="Kelompok A (Umum)" class="block w-full mt-1" required />
            </div>
            <div>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-teal-bg/30">
                    ...
                    <td class="px-4 py-3 text-right">
                        ...
                        <form method="POST" action="{{ route('tu.kelompok-mapel.destroy', $k) }}" class="inline" onsubmit="return confirm('Hapus?')">
                            @csrf @method('DELETE')
                ...
                <tr id="edit-{{ $k->id }}" class="hidden bg-surface-base/50">
                    <td colspan="3" class="px-4 py-3">
                        <form method="POST" action="{{ route('tu.kelompok-mapel.update', $k) }}" class="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
                            @csrf @method('PUT')
                            <div><x-text-input name="nama" :value="$k->nama" class="block w-full mt-1" required /></div>
                            <div><x-text-input name="keterangan" :value="$k->keterangan" class="block w-full mt-1" /></div>
