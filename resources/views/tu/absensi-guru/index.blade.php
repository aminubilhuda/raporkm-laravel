...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-card shadow-card p-5 md:p-6 border-l-[6px] border-l-coral">
        ...
        <form id="absensiForm" method="POST" action="{{ route('tu.absensi-guru.check-in') }}" enctype="multipart/form-data">
            @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function submitCheckIn() {
        ...
        try {
            const response = await fetch('{{ route("tu.absensi-guru.check-in") }}', {
                method: 'POST',
                headers: {
    ...
    async function submitCheckOut() {
        ...
        try {
            const response = await fetch('{{ route("tu.absensi-guru.check-out") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value,
                    'Accept': 'application/json'
