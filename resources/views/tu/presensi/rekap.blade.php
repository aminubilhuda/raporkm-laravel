...
@section('content')
<div class="space-y-6">
<div><h1 class="text-2xl md:text-3xl font-extrabold text-teal-primary-dark flex items-center gap-2"><x-heroicon-o-clipboard-document-check class="w-7 h-7"/> Rekap Presensi</h1><p class="mt-1 text-sm text-gray-500">Ringkasan kehadiran siswa per kelas.</p></div>
<div class="bg-white rounded-card shadow-card p-5 md:p-6 border-l-[6px] border-l-teal-primary">
    <form method="GET" class="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><select name="kelas_id" onchange="this.form.submit()" class="block w-full border-teal-primary/20 rounded-card"><option value="">Pilih Kelas</option>@foreach($kelass as $k)<option value="{{$k->id}}" {{$kelasId==$k->id?'selected':''}}>{{$k->nama_kelas}}</option>@endforeach</select></div></form>
</div>
@if($kelasId)
